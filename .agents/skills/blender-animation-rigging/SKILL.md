...
3. Apply via Python: `mod = fcurve.modifiers.new('SMOOTH')` then `mod.blend_type = 'GAUSSIAN'` (verify the type identifier against your Blender 5.x build via `bpy.types.FModifier` enum items)
