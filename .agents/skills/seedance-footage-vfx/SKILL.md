...
description: >
  Write or revise Seedance 2.0 video-to-video prompts that transform existing
  footage while preserving the source subject, performance, framing, timing, or
  dialogue. Use when a real clip is the starting point for added VFX, environment
  replacement, creatures, relighting, regrading, timed camera moves, lip-sync,
  runtime changes, or a matching transformed start frame. Also use when the user
  attaches a clip and asks for a Seedance prompt or pastes a footage-edit prompt
  for revision. For a new scene without source footage, use seedance-prompt-20 or
  seedance-prompt-25. For Seedance 2.5 structured editing, use
  seedance-prompt-25.
---
